...
function listenTo(userId, receiver) {
  ...
  // ===== พูดแทรกได้ (barge-in) — เจ้าของสั่ง 5 ส.ค. 2026 =====
  //
  // "ถ้าต้องขัดจังหวะก็ขัดได้เลย ไม่ต้องเกรงใจ เราจะหยุดให้และรับจังหวะคุยต่อได้ทันที"
  //
  // เดิมต้องพูดคำว่า "พอ/หยุด" ถึงจะหยุด ซึ่งต้องรอถอดเสียงก่อน = ช้าไป 3-5 วินาที
  // กว่าจะหยุดเขาก็พูดทับไปครึ่งประโยคแล้ว
  // ตอนนี้ใช้สัญญาณ "เริ่มพูด" ของ Discord ตรง ๆ — รู้ทันทีที่ไมค์เขาเปิด ไม่ต้องรอถอดเสียง
  //
  // ทำไมถึงปลอดภัย: Discord แยก stream ตาม user อยู่แล้ว เสียงที่บอทพูดเองไม่เข้าทางนี้
  // (เสียงจากลำโพงย้อนเข้าไมค์เจ้าของยังนับเป็นเขาพูด — แต่กรณีนั้นเราก็ควรหยุดอยู่ดี)
  if (speaking) {
    try {
      player.stop(true);
      console.log("  → โด้เริ่มพูด หยุดทันที (barge-in)");
    } catch { /* หยุดไม่ได้ก็ปล่อยให้พูดจบ */ }
  }
