...
import { copyReplyText, refreshMuted, isChatMuted, isBrandMuted } from "./lib/notify.mjs";
...
const HEARTBEAT_MS = Number(process.env.OHO_HEARTBEAT_MINUTES || 60) * 60000; // ส่ง "ยังทำงานอยู่" ทุกๆ กี่นาที (ดีฟอลต์ 60)
let lastBeat = Date.now(); // เริ่มนับจากตอนสตาร์ท → heartbeat แรกอีก ~1 ชม. (ไม่ชนข้อความเริ่มเฝ้า)
...
async function tick(page) {
  ...
  for (const w of toInspect) {
    ...
    if (route?.company && isBrandMuted(route.company)) continue; // แบรนด์นี้สั่งปิดแจ้งเตือน → ข้าม
  ...
  // heartbeat: แทนที่จะสแปมข้อความค้างทุกนาที → บอกแค่ว่าระบบยังทำงานอยู่ ทุก 1 ชั่วโมง
  if (nowMs - lastBeat >= HEARTBEAT_MS) {
    lastBeat = nowMs;
    const backlog = waiting.filter((w) => levelFor(w.waitSec) > 0).length;
    const body = backlog > 0
      ? `📥 ตอนนี้มีแชทค้างรอ ${backlog} รายการ · น้องวานดูแลให้อยู่นะคะ`
      : `📥 ตอนนี้ไม่มีแชทค้าง ทุกอย่างเรียบร้อยค่ะ`;
    await tg("sendMessage", { chat_id: ALERT_CHAT, text: `🌐 ระบบเฝ้าแชทยังทำงานปกติค่ะ\n${body}` }).catch(() => {});
  }
