...
กติกาโค้ดที่ห้ามฝืน (อ่าน docs/vex-code-rules.md ก่อนเริ่มเสมอ):
1. ห้ามเพิ่ม regex ตัดสินใจเส้นทางคำสั่ง — เพิ่มความสามารถใหม่ = เพิ่ม entry ใน INTENT_CATALOG (src/lib/kiki-router.ts) ที่เดียว
2. ห้ามเขียนข้อความตอบแบบสำเร็จรูป — ต้องผ่าน askKiki / vexLine / vexSay / vexList เสมอ
   จำเป็นต้องฟิกจริง ๆ ให้เติมคอมเมนต์ // canned-ok: <เหตุผล>
   ก่อน commit ต้องรัน `npm run kiki:check` ให้ผ่าน (นอกเหนือจาก tsc)
3. การกระทำจริงต้องให้ระบบตรวจหลักฐานเองก่อนรายงานว่าสำเร็จ ห้ามให้โมเดลเป็นคนเคลม
4. เก็บลิงก์/รูป/ไฟล์เข้าคลังเฉพาะที่เจ้าของสั่งเท่านั้น

ถ้าสเปกกำกวมให้ตัดสินใจเองแบบ conservative ตามแพตเทิร์นที่มีอยู่ในโค้ด (router-first + ปุ่มยืนยัน + deterministic ก่อน LLM)`;
