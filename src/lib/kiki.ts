...
export async function transcribeAudio(filePath: string, mime = "audio/ogg", opts: { fast?: boolean } = {}): Promise<string> {
  ...
  //
  // เทียบโมเดลใหม่อีกรอบ 5 ส.ค. (รอบก่อนเทียบ 2.5-flash-lite แล้วไม่ผ่าน คราวนี้ 3.1):
  //             ประโยคยาว   คำสั้น 6 คำ
  //   2.5-flash    3.6 วิ      ถูก 2/6
  //   3.1-lite     1.6 วิ      ถูก 3/6   ← เร็วกว่า 2.2 เท่าและแม่นกว่า
  // ถอดเสียงเป็นคอขวดใหญ่สุดของเส้นทางเสียง (3-4 วิจากทั้งหมด 9-10) ลดตรงนี้ได้คุ้มที่สุด
  const isShort = opts.fast ?? buf.length < 12_000;
  const model = process.env.KIKI_STT_MODEL?.trim() || "gemini-3.1-flash-lite-preview";
  ...
  // ===== กันโมเดล "ตอบ" แทนที่จะ "ถอด" =====
  //
  // เจอจริง 5 ส.ค. ตอนเทียบโมเดล — ป้อนเสียงคำว่า "หยุด" แล้วได้กลับมาว่า
  // "ขออภัยครับ คุณยังไม่ได้แนบไฟล์เสียงมาให้ หากคุณต้องการให้ผมถอดความ กรุณาอัปโหลด..."
  // อีกเคสได้ "ถอดข้อความจากเสียงนี้" (มันอ่านคำสั่งของเราออกมาเฉย ๆ)
  // ถ้าปล่อยผ่าน ข้อความพวกนี้จะกลายเป็น "คำสั่งของเจ้าของ" แล้ววิ่งเข้าสมองทั้งดุ้น
  //
  // จับด้วยความยาวเทียบเวลาพูดจริง: คนพูดไทยได้ราว 12 ตัวอักษร/วินาที
  // ถอดออกมายาวเกินสามเท่าของที่เป็นไปได้ = ไม่ใช่คำถอดแน่นอน
  const spokenSec = buf.length / 3000; // OGG/Opus 24k ≈ 3 KB ต่อวินาที
  const tooLong = text.length > Math.max(40, spokenSec * 12 * 3);
  const isModelChatter = /^(ขออภัย|ขอโทษ|ฉันไม่|ผมไม่|I'm sorry|I cannot|Sorry)/i.test(text)
    || /กรุณาอัปโหลด|ยังไม่ได้แนบ|ไม่มีไฟล์เสียง|ถอดข้อความจากเสียงนี้/.test(text);
  if (tooLong || isModelChatter) throw new Error("ถอดเสียงไม่ได้ (โมเดลตอบแทนการถอด)");
...
--- ห้ามอ้างว่า "กำลังรอ/กำลังหา" ในสิ่งที่ตัวเองต้องตอบเดี๋ยวนี้ ---
คุณคือคนตอบ ไม่ใช่คนรอคนอื่นตอบ
ถ้าในบริบทมี "เรื่องที่กำลังค้างอยู่" ที่ตรงกับคำถามนี้ นั่นคือ **งานของคุณเอง** ที่กำลังทำอยู่ตอนนี้
ห้ามพูดว่า "ผมยังรอข้อมูลจากระบบอยู่" · "รอผลอยู่นะครับ" · "พอได้แล้วจะแจ้ง" เด็ดขาด
มีข้อมูลก็ตอบ ไม่มีก็บอกตรง ๆ ว่าไม่มี — สองทางนี้เท่านั้น
(เคสจริง 5 ส.ค.: ตอบว่า "ยังรอข้อมูลพยากรณ์ฝนจากระบบอยู่" แล้วเจ้าของรอเก้อ
 ก่อนที่ตัวเองจะมาสารภาพทีหลังว่า "ความจริงคือไม่มีอะไรรันอยู่เลย ผมพูดลอย ๆ ไปเอง")
...
ห้ามเด็ดขาด: เปลี่ยน/เพิ่ม/ลดตัวเลข ชื่อคน ชื่อไฟล์ ลิงก์ วันเวลา · เพิ่มข้อเท็จจริงใหม่ · ตัดเนื้อหาทิ้ง
ห้ามตกแต่งเกินกฎ: ห้ามใส่เครื่องหมายคำพูดครอบชื่อรายการที่ต้นฉบับไม่มี · ห้ามเพิ่มสัญลักษณ์นำหน้าที่ไม่ได้สั่งไว้
