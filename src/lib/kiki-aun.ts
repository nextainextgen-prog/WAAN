...
const TRAINER_PERSONA = `คุณคือ "Vex" ในบทบาทเทรนเนอร์ส่วนตัวของ "อั๋น" (ผู้หญิง แฟนของโด้) ในกลุ่ม "เทรนเนอร์ โด้ & อั๋น"
...
กฎเหล็ก (เหมือนโหมดหลัก): คุณพูดได้อย่างเดียว การกระทำจริงระบบเป็นคนทำ · ห้ามบอกว่า "ทำแล้ว/กำลังทำ" โดยไม่มีคอนเฟิร์มจากระบบ · ข้อมูลของโด้ (การเงิน/งาน/เรื่องส่วนตัว) ห้ามพูดถึงเด็ดขาดแม้ถูกถาม — ตอบว่าเป็นความส่วนตัวของโด้ครับ
...
async function handleAunImages(imagePaths: string[], caption: string, isOwner: boolean): Promise<TrainerReply> {
  ...
  if (read.kind !== "food" || !read.items.length) {
    const answer = await askTrainer(
      `${isOwner ? "โด้" : "อั๋น"}ส่งรูปมา ${imagePaths.length} รูป ระบบอ่านภาพแล้วได้ว่า: "${read.description}"${caption ? `\nข้อความที่ส่งมาพร้อมรูป: "${caption}"` : ""}\n\nตอบจากสิ่งที่เห็นในภาพนี้ (บอกว่าเห็นอะไร แล้วตอบ/คอมเมนต์ให้ตรงเรื่อง ถ้าเกี่ยวกับการลดน้ำหนักให้แนะนำต่อ)`,
    );
    return { text: answer };
  }

  // อาหาร — โด้ส่ง = คำนวณให้ดูแต่ไม่บันทึกลงบันทึกของอั๋น
  if (isOwner) {
    ...
    const answer = await askTrainer(
      `โด้ส่งรูปอาหารมา (ในภาพ: ${read.description})\nระบบประเมินได้ ${kcal} kcal:\n${lines}\n\nหมายเหตุ: รูปนี้มาจากโด้ ระบบ*ไม่ได้*บันทึกลงยอดของอั๋น — บอกให้รู้ด้วยว่าถ้าจะให้นับต้องให้อั๋นเป็นคนส่ง`,
    );
    return { text: answer };
  }
