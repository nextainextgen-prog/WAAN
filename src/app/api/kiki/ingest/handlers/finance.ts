...
export const pendingAnswerHandler: Handler = async (ctx) => {
  ...
  // ตัวเลขนำหน้าต้องเป็น "ยอดเงิน" จริง ไม่ใช่เวลาหรือจำนวนนับ
  //
  // เคสจริงที่พัง 5 ส.ค. 2026: ถามด้วยเสียงว่า "หกโมงเย็นวันนี้ฝนจะตกไหม"
  // ตัวถอดเสียงคืน "6 โมง เย็น วัน นี้ ฝน จะ ตก ไหม" → ขึ้นต้นด้วยเลขแล้วเว้นวรรค
  // → ชนกฎ "319 ค่าตั๋วหนัง" → คำถามเรื่องอากาศถูกลากไปเข้าเส้นทางจัดหมวดเงิน
  // (กระทบฝั่งพิมพ์ด้วย ไม่ใช่แค่เสียง — "6 โมงเย็นนัดใคร" ก็ชนเหมือนกัน)
  const isQuestion = /ไหม|มั้ย|หรือเปล่า|รึเปล่า|ป่ะ|\?|อะไร|ทำไม|ยังไง|เมื่อไหร่|ที่ไหน|ใคร|กี่/.test(text);
  const numberIsTime = /^\s*[\d,]+(\.\d+)?\s*(โมง|นาฬิกา|ทุ่ม|น\.|นาที|ชั่วโมง|วัน|เดือน|ปี|ครั้ง|รอบ|คน|อัน|ชิ้น)/.test(text);
  const amountLed = /^\s*[\d,]+(\.\d+)?\s+\S/.test(text) && !isQuestion && !numberIsTime;
  ...
  if (
    ...
    (!imageFiles.length && (await hasPendingTxn()) &&
      (/^(ค่า|เป็นค่า|มันคือ|อันนี้(คือ|เป็น)?|หมวด)/.test(text) ||
        amountLed || // "319 ค่าตั๋วหนัง" — บอกยอดนำหน้า จับคู่รายการจากยอด
