...
export const macHandler: Handler = async (ctx) => {
  const { text, imageFiles, msgId, route, is, reply } = ctx;

  // ===== เจ้าของแนบรูปมาเอง = ถามถึง "รูปนั้น" ไม่ใช่สั่งให้ไปส่องจอเครื่อง =====
  // เคสจริง 6 ส.ค. 2026 เวลา 16:39: ส่งรูปมาถาม "ดูรูปนี้แล้วบอกหน่อยว่าเห็นอะไร"
  // → ตัวอ่านเจตนาเลือก mac → ระบบไปแคปหน้าจอ Warp ของตัวเองมาบรรยายแทน
  //   ตอบยาวเป็นเรื่องเป็นราวแต่ไม่เกี่ยวกับรูปที่ส่งมาเลยสักตัวอักษร
  // ห้ามแก้ด้วยการเขียนกำชับในคำสั่งของตัวอ่านเจตนาอย่างเดียว — "มีไฟล์รูปแนบมาไหม"
  // เป็นข้อเท็จจริงที่ระบบรู้แน่นอน ไม่ใช่เรื่องที่ต้องตีความ
  // ยกเว้นพูดถึงเครื่องชัด ๆ ("แคปหน้าจอ" "เปิดแอป") ซึ่งแนบรูปประกอบมาด้วยได้
  if (imageFiles.length && !MAC_RE.test(text)) return null;
