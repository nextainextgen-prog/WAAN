...
## 11. อัปเดตตอนลงมือจริง (17 ก.ค. 2026 — เฟส 1 เสร็จ)

สิ่งที่ **เปลี่ยนจากสเปกข้างบน** หลังเจอของจริง:

### 11.1 ไม่ต้องพิมพ์ค้นหาเลย — ทั้ง 2 หน้ารับ query param (verify แล้ว)

| หน้า | URL | ผลจริง |
|---|---|---|
| จัดการขอคืนเงิน | `/admin/refund?id=2677` | กรอกช่อง "ไอดี" ให้เอง → เหลือ 1 แถว |
| ประวัติบริการ | `/admin/logs/service?id=175979` | กรอกช่อง "ไอดีประวัติ" ให้เอง → เหลือ 1 แถว |

เจอเพราะคอลัมน์ "ประวัติบริการ" บนหน้า refund เป็นลิงก์ `<a href="/admin/logs/service/?id=175979">` ที่ระบบสร้างเอง
**ผลคือตัดกับดัก SPA controlled input ทิ้งทั้งอัน** (บั๊กที่เคยทำ thunder_expiry พังหลายรอบ: `.fill()` ไม่ bind model, ต้อง `pressSequentially`, ต้องรอ count ที่ไม่ใช่ 0) — ฟีเจอร์นี้ไม่แตะช่องค้นหาเลยแม้แต่ครั้งเดียว

### 11.2 เมทริกซ์จริง = 8 จุด แต่เทียบได้ 6 เมื่อไม่มี noti

`ชื่อร้านค้า` กับ `ไอดีบริการ` ต้องมี noti ถึงจะเทียบได้ (หน้า refund ไม่มี 2 คอลัมน์นี้) → ตอนไม่มี noti การ์ดจะ **ซ่อนคอลัมน์ "บอทแจ้งเตือน" อัตโนมัติ** และรายงานเป็น `ตรวจผ่าน 5/6 จุด` ไม่ใช่ 8

### 11.3 ผลทดสอบจริง (เคส #2677 ของโด้)

```
OK   [hard] ไอดีประวัติบริการ  #175979 = #175979
OK   [hard] ยูสเซอร์          siwaul = siwaul
OK   [hard] ยอดเงิน           ฿1,599.00 = ฿1,599.00
OK   [hard] ยอดในลิงก์ประวัติ  ฿1,599.00 = ฿1,599.00
FAIL [hard] สถานะคำขอ         "อนุมัติแล้ว" (ต้องเป็น "รออนุมัติ")   ← ถูกต้อง! เคสนี้คืนไปแล้ว
--   [hard] ชื่อร้านค้า        (ต้องมี noti ถึงเทียบได้)
OK   [soft] ประเภทรายการ      ต่ออายุผิด ↔ RENEW
```
การ์ดขึ้น "พบข้อผิดพลาด" + ไม่มีปุ่มคืน = **กันคืนซ้ำรายการที่คืนไปแล้วทำงานจริง**

### 11.4 ภาพยืนยัน

`shotRefundRow()` เปิดหน้ารายการ **ไม่กรอง** แล้วตีกรอบแดงแถวเป้าหมาย → ได้สัดส่วนตาม Image 24 เป๊ะ (เห็น "มีข้อมูล 2251 รายการ" + dropdown 10 + ตาราง 10 แถว + กรอบแดงที่แถว 2677) · ถ้าคำขอเก่าจนหลุดหน้าแรก → fallback กรอง `?id=` (ได้แถวเดียว)

### 11.5 ไฟล์จริงที่สร้าง/แก้

| ไฟล์ | สถานะ |
|---|---|
| `src/lib/thunder-refund.ts` | ใหม่ — listPendingRefunds / fetchRefundRow / fetchServiceLog / shotRefundRow (approveRefund = เฟส 2) |
| `src/lib/refund-check.ts` | ใหม่ — เมทริกซ์ 8 จุด + การ์ด HTML→PNG |
| `src/lib/refund-store.ts` | ใหม่ — state machine + `lockForApprove` (atomic guard กันกดซ้อน) |
| `src/lib/refund-notify.ts` | ใหม่ — parser noti (เทสผ่านข้อความจริง 2 เคส) |
| `src/lib/refund-flow.ts` | ใหม่ — pollRefunds + ข้อความกลุ่ม + handleRefundButton |
| `scripts/refund-watch.mjs` | ใหม่ — poller 2 นาที (`npm run refund:watch`) |
| `api/telegram/refund-poll` · `refund-notify` | ใหม่ |
| `prisma/schema.prisma` | + model RefundCase (db:push แล้ว) |
| `roles.ts` · `dedicated-groups` · `ingest` (ปุ่มเลือกหน้าที่) · `callback` · `telegram-bot.mjs` | แก้ — รองรับ `thunder_refund` + ปุ่ม `tref:` + hook noti |
| `run-waan.command` · `stop-waan.command` | + refund watcher (autostart) |
| `telegram.ts` | `tgSendPhoto` รับ extra (parse_mode/reply_markup) |

### 11.6 เหลือทำ

- **ผูกกลุ่ม** — poller คืน `no_group` จนกว่าโด้จะผูก (พิมพ์ "วานเชื่อมกลุ่มนี้" ในกลุ่ม → กดปุ่ม "💳 คืนเครดิต Thunder" ที่ห้อง Lead)
- **วัด rate-limit จริง** — poll 2 นาที = 30 ครั้ง/ชม. เกินจุดที่ thunder_expiry เคยโดน (~15/ชม.) ถ้าโดนให้ตั้ง `REFUND_POLL_SECONDS=300`

---

## 12. เฟส 2 — write path (เขียนครบแล้ว 17 ก.ค. · รอ verify)

### 12.1 `approveRefund(refundId, expect)` — ป้องกัน 4 ชั้นก่อนแตะเงินลูกค้า

| ชั้น | ตรวจอะไร | ไม่ผ่านแล้วทำอะไร |
|---|---|---|
| 1 | เปิด `?id=<refundId>` เจาะจง ต้องได้ **แถวเดียว** | `not_found` / `multiple_rows` — ไม่กด |
| 2 | re-verify สด: ยัง `รออนุมัติ` + ยูสเซอร์/ยอด/historyId ตรงกับตอนแจ้ง | `stale` (มีคนจัดการไปแล้ว) / `wrong_row` — ไม่กด |
| 3 | เมนูที่เปิดต้องมีข้อความ **`จัดการ #<refundId>` ตรงเลข** | กด Escape → `wrong_row` — ไม่กด |
| 4 | คลิกเฉพาะปุ่มข้อความ `^อนุมัติ$` เป๊ะ (ข้าม `ปฏิเสธ`/`ยกเลิก` ชัดเจน) | `approve_item_not_found` — ไม่กด |
| ยืนยัน | `waitForFunction` จนสถานะแถวนั้นเป็น **`อนุมัติแล้ว`** จริง (15 วิ) | `status_unchanged` — รายงานว่าไม่สำเร็จ ไม่โกหกว่าคืนแล้ว |

> หลักคิด: **คลิกติด ≠ ระบบ save แล้ว** — ต้องให้ระบบยืนยันสถานะเองเท่านั้น

### 12.2 อย่างอื่นที่เพิ่ม

- **`sweepWaiting()`** — เตือนซ้ำ 1 ครั้งที่ 30 นาที (reply ข้อความเดิม) · ปิดปุ่ม + `expired` ที่ 6 ชม. · รันทุกรอบ poll และไม่พึ่ง browser (ทำงานแม้ session หลังบ้านหมด)
- **แท็กคนขอ** — `findMemberByName(requestedBy)` → `tg://user?id=` (ถ้าไม่มีใน roster ใช้ `@username` ตรงๆ)
- **`thunderSessionExpiry()`** — อ่าน `exp` จาก JWT `x-auth` ใน localStorage ของ storageState ตรงๆ ไม่ต้องเปิด browser → **เตือนล่วงหน้า 12 ชม. ก่อน session หมด** (เตือนครั้งเดียวต่อ token)

### 12.3 🔑 ค้นพบเรื่อง session (แก้ความเข้าใจผิดเดิม)

- auth ของ Thunder = **JWT `x-auth` ใน localStorage** (ไม่ใช่ cookie) → อยู่ในส่วน `origins` ของ storageState
- **อายุ 3 วันเป๊ะ** ไม่ใช่ 1-1.5 ชม. อย่างที่เคยเข้าใจ (ไฟล์สร้าง 14 ก.ค. 17:14:48 → หมด 17 ก.ค. 17:14:48)
- อาการ session หมด: **HTTP 200 แต่ body ว่างเปล่าสนิท** (ไม่มี thead ไม่มีข้อความใดๆ) — อย่าสับสนกับ rate-limit ที่จะคืน "มีข้อมูล 0 รายการ" แต่ยังมีโครงหน้า

### 12.4 ✅ ผลทดสอบท่อปุ่ม end-to-end (17 ก.ค. 17:40)

จำลองคำขอ #2677 (สถานะจริงในระบบ = "อนุมัติแล้ว") ส่งเข้าแชทโด้พร้อมปุ่มจริง → โด้กด "คืนเลย" → ผลจาก DB:

```
state:      stale            ← ชั้น 2 (re-verify สด) จับได้ว่าไม่ใช่ "รออนุมัติ" แล้ว → ไม่กด
decidedBy:  T3 (7750653134)  ← lockForApprove ทำงาน รู้ว่าใครกด
error:      null
activity:   [refund-stale] warn · คำขอ #2677 ถูกจัดการในระบบไปแล้วก่อนที่ T3 จะกดคืน (สถานะ อนุมัติแล้ว)
```

**พิสูจน์แล้ว:** Telegram → bot branch `tref:` → callback route → เช็คสิทธิ์ → lock → เปิดหน้าจริง → re-verify → หยุด → logActivity → ตอบกลับ (ครบทั้งเส้น ไม่มีการเขียนเกิดขึ้น) · ปุ่ม "ดูรายละเอียด" (ไม่ปิดปุ่ม) และ "ไม่ต้อง" ใช้ท่อเดียวกัน
*(ลบเคสทดสอบ + activity ทดสอบออกจาก DB แล้ว — RefundCase = 0 แถว)*

### 12.5 🔴 สิ่งที่ยัง verify ไม่ได้ (ต้องทำก่อนใช้จริง)

**selector ของ popup badge ยังไม่เคยเห็น DOM จริง** — เมนู "จัดการ #xxxx / ✅ อนุมัติ / ❌ ปฏิเสธ" เปิดได้เฉพาะแถวสถานะ `รออนุมัติ` เท่านั้น (ทดลองคลิก badge แถว `อนุมัติแล้ว` แล้ว → ไม่มีเมนูโผล่) และช่วงที่พัฒนา **ไม่มีคำขอรออนุมัติในระบบเลยสักรายการ** → เขียนจากภาพที่โด้ส่ง (Image 22/23) + แพตเทิร์น Mantine Menu

ผลถ้า selector ไม่ตรง: วาน **ไม่กดอะไรเลย** แล้วรายงาน `menu_not_found` / `approve_item_not_found` → ปลอดภัย (fail closed) แต่ต้องมาแก้ selector

**วิธี verify:** พอมีคำขอจริงเข้ามา (หรือโด้สร้าง test request) → เปิดเมนูอ่าน DOM ก่อน แล้วค่อยให้แอดมินกดปุ่มจริงเป็นการทดสอบ write ครั้งแรก (เหมือนที่ thunder_expiry ทำ)
